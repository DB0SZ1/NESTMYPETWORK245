...
// Fetch all users who are sitters or pending sitters
$sitters = [];
try {
    $stmt = $pdo->query("SELECT id, fullname, email, sitter_status, created_at FROM users WHERE is_sitter = 1 ORDER BY created_at DESC");
...
$pageTitle = "Sitter Verifications";
?>
    ...
    <div class="dashboard-layout">
        <aside class="sidebar">
            ...
            <nav class="sidebar-nav">
                ...
                <a href="admin_verifications.php" class="nav-item active"><i class="fas fa-check-circle"></i><span>Verifications</span></a>
        ...
        <main class="main-content">
            <header class="main-header">
                <div class="header-greeting">
                    <h1>Sitter Verifications</h1>
                    <p>Approve or reject sitter applications.</p>
                </div>
            </header>
                        ...
                        <table>
                            ...
                            <tbody>
                                <?php if (empty($sitters)): ?>
                                    <tr><td colspan="4" style="text-align: center;">No sitters found.</td></tr>
                                <?php endif; ?>
                                <?php foreach ($sitters as $sitter): ?>
                                    <tr>
                                        ...
                                        <td class="action-buttons">
                                            <?php if ($sitter['sitter_status'] == 'pending'): ?>
                                                <a href="process_verification.php?id=<?php echo $sitter['id']; ?>&action=approve" class="btn-action approve">Approve</a>
                                                <a href="process_verification.php?id=<?php echo $sitter['id']; ?>&action=reject" class="btn-action reject">Reject</a>
                                            <?php elseif ($sitter['sitter_status'] == 'approved'): ?>
                                                <a href="process_verification.php?id=<?php echo $sitter['id']; ?>&action=reject" class="btn-action reject">Suspend</a>
                                            <?php elseif ($sitter['sitter_status'] == 'rejected'): ?>
                                                <a href="process_verification.php?id=<?php echo $sitter['id']; ?>&action=approve" class="btn-action approve">Re-Approve</a>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
